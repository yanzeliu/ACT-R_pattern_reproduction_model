;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Take snapshot first to simuluate that human has the ability to get an overall
;;; impression of the whole stimulus at the first sight.
;;; The model will then start to recognize patterns from discs while stimulus is visible.
...
(define-model l-chunking
    ...
    (chunk-type task-goal state found-pattern root pre-root-slot pre-root-slot-value cur-tar-root-slot rehearsing-disc next-group-slot check-screen
    ...
    (add-dm
        (goal isa task-goal state start root root-chunk )
    )

    (goal-focus goal)

    ;;;==============================================================
    ;;;     Before stimulus stage
    ;;;==============================================================
    ...
    (P find-stimulus-disappear-after-attention-shifting
        =goal>
            ISA                 task-goal
            state               swifting-eye-to-first-disc
        ?visual>
            state               free
        =visual>
            value               =visual-obj-value
        !eval!                  (OR (string= =visual-obj-value "mask") (string= =visual-obj-value "X"))
    ==>
    ...
    (P find-mask-disappeared-by-scene-change
        =goal>
            check-screen        checking-mask
        ?visual>
            scene-change        t
        ...
        ; =visual>
        ;     value               =visual-obj-value
        ; !eval!                  (not (string= =visual-obj-value "mask"))
    ==>
    ...
    ; (P find-mask-disappeared-by-visual-buffer-content
    ;     =goal>
    ;         check-screen        checking-mask
    ; ==>
    ; )
    ...
    ;;; The model find there is no chunking structure in DM.
    ;;; It will then start to retrieve activated disc from DM instead
    ;;; to do the response.
    (P retrieve-activated-disc-locaiton-response
        =goal>
            state               response-from-root
            pre-root-slot       nil
    ==>
