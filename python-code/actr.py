...
class interface():
    ...
    def send_base_level(self, chunk, level):
        d = {}
        r = request(self.cmd_id)
        self.actions[self.cmd_id] = r
        ...
        self.cmd_id += 1
        self.id_lock.release()
        d['params'] = ('set-base-levels', False, (chunk, level))
...
"""
    customized function for set-based-levels and get-based-levels
"""
# def add_dm_chunks(*chunks):
    # return current_connection.evaluate_single("set-base-levels", chunks)

def set_base_level(chunk, level):
    return current_connection.evaluate_single("set-chunk-reference-count", chunk, level)

def set_creation_time(chunk, time):
    return current_connection.evaluate_single("set-chunk-creation-time", chunk, time)

def set_base_levels_fct(prarms):
    return current_connection.evaluate_single("set-base-levels-fct", prarms)

def get_base_level(*prarms):
    return current_connection.evaluate_single("get-base-level", *prarms)

def add_dm_chunks(*prarms):
    return  current_connection.evaluate_single("add-dm-chunks", *prarms)

def set_cursor_location(*prarms):
    return current_connection.evaluate_single("set-cursor-position", *prarms)